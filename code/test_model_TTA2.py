...
#             data_transform = self.data_transform
            ...
            for pictures, inputs, _, labels in tqdm(test_dataloader):
                ...
                if n_TTA >= 1:                           # means we will use TTA
                    for i in tqdm(range(len(labels))):
                        ...
                        #outputs1 = model_test(inputs[i])        # outputs1 = torch.Tensor = torch.Size([32, 24]) .. 32=n_TTA
                        ...
                        preds = [np.argmax(output) for output in outputs]
                        preds_count = np.bincount(preds)/n_TTA
                        preds_v_entropy = np.sum(entr(preds_count))
                        preds_rate = preds_count[np.argmax(preds_count)]
                        ...
                        l_v_entropy.append(preds_v_entropy)
                        l_preds_rate.append(preds_rate)
        ...
        df_result = pd.DataFrame({'picture': l_pictures, 'label': l_labels, 'model': l_model, 
                                  'outputs': l_outputs, 'v_entropy': l_v_entropy, 'preds_rate': l_preds_rate})
        ...
        for pic in tqdm(pic_list):
            if model_type == 'single':
                ...
                preds_rate = np.array(df_result[df_result['picture'] == pic]['preds_rate'])
                v_entropy = np.array(df_result[df_result['picture'] == pic]['v_entropy'])

                df_result_ensemble.loc[counter] = [pic, tmp_model, tmp_label, tmp_outputs_all, preds_rate, v_entropy]
                ...
                for model in model_list:
                    ...
                    preds_rate = np.array(df_result[(df_result['picture'] == pic) & 
                              (df_result['model'].str.contains(model, na=False))]['preds_rate'])
                    v_entropy = np.array(df_result[(df_result['picture'] == pic) & 
                              (df_result['model'].str.contains(model, na=False))]['v_entropy'])
