...
class MyImageDataset(Dataset):
    def __init__(self, config, dops1, class_to_idx=None, t_data_set = 'train', use_seed = False):
        ...
        #l_hard_chrmsm = ['8', '13', '14', '15', '21', '22', 'y']
        l_hard_chrmsm = ['14', '15', '21', '22', 'y']
        self.img_labels=self.img_labels[pd.DataFrame(self.img_labels.label.tolist()).isin(l_hard_chrmsm).any(1).values]
        ...
        if class_to_idx == None:
            if self.input_type == 'int_str':
                #l_label = [8, 13, 14, 15, 21, 22, 'y']
                l_label = [14, 15, 21, 22, 'y']
                #l_index = [7, 12, 13, 14, 20, 21, 23]
                l_index = [0, 1, 2, 3, 4]
            elif self.input_type == 'int':
                #l_label = [8, 13, 14, 15, 21, 22, 24]
                #l_index = [7, 12, 13, 14, 20, 21, 23]
                l_label = [14, 15, 21, 22, 'y']
                l_index = [0, 1, 2, 3, 4]
